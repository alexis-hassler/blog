...
D'ailleurs, je n'ai pas eu à chercher très loin puisque la technique est décrite dans la link:https://tomcat.apache.org/tomcat-8.0-doc/realm-howto.html#Digested_Passwords[doc de Tomcat].
...
C'est d'ailleurs désigné comme tel dans un link:https://cwiki.apache.org/confluence/display/TOMCAT/Password[FAQ de Tomcat] et c'est la raison pour laquelle Tomcat n'implémente aucune solution de chiffrement.
...
Il suffit d'utiliser la DataSource de link:https://tomcat.apache.org/tomcat-8.0-doc/jdbc-pool.html[Tomcat JDBC], en ajoutant à notre configurationde DataSource l'attribut `factory=org.apache.tomcat.jdbc.pool.DataSourceFactory`. 
...
Vous pouvez aussi récupérer le fichier jar sur Maven Central : link:https://repo1.maven.org/maven2/fr/sewatech/utils/tc-utils/[fr.sewatech.utils:tc-utils]. 
